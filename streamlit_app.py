    ...
    if uploaded_file and question:
        ...
        for col in df.columns:
            ...
            if pd.api.types.is_numeric_dtype(df[col]):
                numeric_col = col
            if df[col].dtype == 'object' and 'category' in col.lower():
                category_col = col
        ...
        if date_col and numeric_col:
            ...
            parsed_data = {labels[i]: int(numbers[i]) for i in range(min(len(labels), len(numbers)))}
            ...
            fig = go.Figure()
            if chart_type == "Bar":
                fig.add_trace(go.Bar(x=list(parsed_data.keys()), y=list(parsed_data.values()), marker_color="#00f900"))
            elif chart_type == "Line":
                fig.add_trace(go.Scatter(x=list(parsed_data.keys()), y=list(parsed_data.values()), mode='lines', line=dict(color="#00f900")))
            elif chart_type == "Pie":
                fig.add_trace(go.Pie(labels=list(parsed_data.keys()), values=list(parsed_data.values())))
