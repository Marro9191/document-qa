        ...
        if df is not None:
            ...
            if show_full_table:
                ...
                if months and numbers:
                    # Match numbers to months (assuming numbers follow months in the response)
                    for i, month in enumerate(months[:len(numbers)]):  # Limit to number of found numbers
                        parsed_data[month.capitalize()] = int(numbers[i])
                ...
                # Identify date and numeric columns dynamically
                for col in df.columns:
                    if any(keyword in col.lower() for keyword in keywords + ['date', 'time']):
                        date_col = col
                    if any(keyword in col.lower() for keyword in keywords + ['review', 'number', 'count', 'total', 'value']):
                ...
                if date_col and numeric_col:
                    # Filter for relevant rows (e.g., Toothbrush category if mentioned in query)
                    ...
                    # Convert date column to datetime with flexible format (DD/MM/YY as default)
                    try:
                        ...
                        # Try without specific format if DD/MM/YY fails
                ...
                if not filtered_df.empty and date_col and numeric_col:
                    ...
                    response_months = [month for month in parsed_data.keys()]
                    df_months = filtered_df[date_col].unique().tolist()

                    # Find matching months and values in the DataFrame
                    chart_data = {}
                    for month in response_months:
                        ...
                        if month_str in df_months:
                            chart_data[month] = parsed_data[month.capitalize()]

                    if chart_data:
                        # Generate a bar chart for monthly comparisons (matching the response)
                        fig = go.Figure()
                        ...
                        title = f"Total {numeric_col.capitalize()} Comparison Over {date_col}"
                        fig.update_layout(
                            title=title,
                            xaxis_title=date_col,
                            yaxis_title=numeric_col.capitalize(),
                            height=500,
                            width=700
                        )
